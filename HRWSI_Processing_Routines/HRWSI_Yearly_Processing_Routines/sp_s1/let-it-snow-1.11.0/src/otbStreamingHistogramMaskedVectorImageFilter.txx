...
PersistentHistogramMaskedVectorImageFilter<TInputImage, TMaskImage>
  ...
  m_SubSamplingRate(1)
{
  ...
  m_Size.Fill(255);
  ///mask value
  this->SetMaskValue( itk::NumericTraits<MaskPixelType>::max() );
...
template<class TInputImage, class TMaskImage>
typename PersistentHistogramMaskedVectorImageFilter<TInputImage, TMaskImage>::HistogramListType*
PersistentHistogramMaskedVectorImageFilter<TInputImage, TMaskImage>
...
template<class TInputImage, class TMaskImage>
const typename PersistentHistogramMaskedVectorImageFilter<TInputImage, TMaskImage>::HistogramListType*
PersistentHistogramMaskedVectorImageFilter<TInputImage, TMaskImage>
...
::Reset()
{
  ...
  //MG reset input mask?
  //TMaskImage * inputPtr = const_cast<TMaskImage *>(this->GetInput());
  //inputPtr->UpdateOutputInformation();
...
::ThreadedGenerateData(const RegionType& outputRegionForThread, itk::ThreadIdType threadId)
{
  ...
  //get mask image	 
  itk::ImageRegionConstIterator< TMaskImage > maskIt( this->GetMaskImage(), outputRegionForThread );
  it.GoToBegin();
  maskIt.GoToBegin();
...
void
PersistentHistogramMaskedVectorImageFilter<TImage, TMaskImage>
