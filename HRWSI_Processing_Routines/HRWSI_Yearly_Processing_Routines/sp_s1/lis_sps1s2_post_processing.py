...
class LisSpS2PostProcessing:
    ...
    BASE_FUW_MASK_FOLDER = 'fuw'
    BASE_NM_MASK_FOLDER = 'nm'
    BASE_WATER_MASK_FILE_NAME = 'WL_2018_60m'
    BASE_TCD_MASK_FILE_NAME = 'TCD_2018_010m_eu_03035_V2_0_60m'
    BASE_FUW_MASK_FILE_NAME = '60m_MASK_FOREST_URBAN_WATER_v20240404'
    BASE_NM_MASK_FILE_NAME = '60m_MASK_NON_MOUNTAIN_AREA_V20211119'
    BASE_L2A_FOLDER = 'L2A'
    ...
    postprocessing_to_edit_files = {
        ...
        'NCSO': {
            'src_name': '',
            'src_path': f'{ALGO_OUTPUT_FOLDER_NAME}',
            'dst_suffix_name': 'NCSO.tif'
        },
        'NWSO': {
            'src_name': '',
            'src_path': f'{ALGO_OUTPUT_FOLDER_NAME}',
            'dst_suffix_name': 'NWSO.tif'
        }
    }
    ...
    def __init__(
        self, workdir_path, input_folder, output_folder, tile_id,
        start_date, end_date
    ) -> None:
        ...
        self.postprocessing_auxilliary_files['FUW']['src_name'] = f'T{tile_id}_{self.BASE_FUW_MASK_FILE_NAME}.tif'
        self.postprocessing_auxilliary_files['FUW']['src_path'] = f'{self.BASE_FUW_MASK_FOLDER}/{tile_id}'

        self.postprocessing_auxilliary_files['NM']['src_name'] = f'T{tile_id}_{self.BASE_NM_MASK_FILE_NAME}.tif'
        self.postprocessing_auxilliary_files['NM']['src_path'] = f'{self.BASE_NM_MASK_FOLDER}/{tile_id}'

        self.product_folder_name_final = f'{self.product_prefix_name}_S1-S2_{start_date}-{end_date}_T{tile_id}_{self.product_version_id}'
        self.product_folder_name = f'{self.product_folder_name_final}_tmp'

        for key, value, in self.postprocessing_to_edit_files.items():
            value['src_name'] = f'{self.ALGO_STATIC_OUTPUT_PREFIX_NAME}-{key}_T{tile_id}_{start_date}_{end_date}_{self.ALGO_VERSION}_1.tif'
        ...
        self.update_post_processing_input_files()
        self.read_input_output_dates()

    def update_post_processing_input_files(self):
        # manupilations for s1s2
        for postprocessing_dict in self.postprocessing_dictionnaries_input_files:
            for value in postprocessing_dict.values():
                value['src_path'] = value['src_path'].replace(self.ALGO_OUTPUT_FOLDER_NAME,os.path.join(self.ALGO_OUTPUT_FOLDER_NAME,self.output_folder))
                value['src_name'] = value['src_name'].replace('_T'+self.tile_id+'_','_'+self.tile_id+'_')
                if 'dst_suffix_name' in value and value['dst_suffix_name'] in ['NWSO.tif','NCSO.tif']:
                    file_preprocessing_path = os.path.join(
                        self.workdir_path,
                        value['src_path'],
                        value['dst_suffix_name']
                    )
                    ...
                    if os.path.exists(file_preprocessing_path):
                        if os.path.exists(file_path):
                            os.remove(file_path)
                        os.rename(file_preprocessing_path,file_path)

        self.ALGO_INPUT_FOLDER_NAME = os.path.join(self.ALGO_INPUT_FOLDER_NAME,self.input_folder)

        # print(self.product_folder_name_final,self.product_folder_name)
        self.product_folder_name = os.path.join('tmp', self.product_folder_name)
        # print(self.product_folder_name_final,self.product_folder_name)

        # in script manipulations:
        # rewrite_cog.rewrite_cog : self.workdir_path, self.product_folder_name_final -> self.workdir_path, self.ALGO_OUTPUT_FOLDER_NAME, self.product_folder_name_final
        # add_quicklook.add_quicklook : self.workdir_path, self.product_folder_name_final -> self.workdir_path, self.ALGO_OUTPUT_FOLDER_NAME, self.product_folder_name_final
        # defining additional arguments (init and argparser): self.tile_id, self.input_folder, self.output_folder
        # input file versions: {self.ALGO_VERSION}.tif' -> {self.ALGO_VERSION}_1.tif'
        # product_version_id = 'V200_1' -> product_version_id = 'V100_1'
    ...
    def make_lis_sps2_qcflags_layer(self):
        ...
        fuw_layer_complete_src_path = os.path.join(
            self.workdir_path,
            self.postprocessing_auxilliary_files['FUW']['src_path'],
            self.postprocessing_auxilliary_files['FUW']['src_name']
        )
        nm_layer_complete_src_path = os.path.join(
            self.workdir_path,
            self.postprocessing_auxilliary_files['NM']['src_path'],
            self.postprocessing_auxilliary_files['NM']['src_name']
        )
        ...
        #TODO: bit number in commment above is incorrect
        source_list += [
            {
                'sources': [
                    {
        ...
        #TODO: bit numbers in commments above is incorrect
        #dont exist in s1s2
        # source_list += [
        #     {
        #         'sources': [
        ...
        #bit 7: SWS detection area
        #WATER:210,FOREST:220,URBAN:230,NODATA:255,MOUNTAIN:0,NONMOUNTAIN:240(includes sea and non-eea38)
        #doesnt exist in s2
        source_list += [
            {
                'sources': [
                    {
                        'filepath': fuw_layer_complete_src_path,
                        'bandnumber': 1,
                        'unpack_bits': False
                    },
                    {
                        'filepath': nm_layer_complete_src_path,
                        'bandnumber': 1,
                        'unpack_bits': False
                    }
                ], \
                'operation': {
                    7: '(A0!=210)*(A0!=220)*(A0!=230)*(A1==0)'
                }
            }
        ]
    ...
    def make_lis_sps2_qc_layer(self):
        ...
        sp_ncso_complete_dst_path = os.path.join(
            self.workdir_path,
            self.product_folder_name,
            f'{self.product_folder_name_final}_{self.postprocessing_to_edit_files["NCSO"]["dst_suffix_name"]}'
        )
        sp_nwso_complete_dst_path = os.path.join(
            self.workdir_path,
            self.product_folder_name,
            f'{self.product_folder_name_final}_{self.postprocessing_to_edit_files["NWSO"]["dst_suffix_name"]}'
        )
        ...
        source_list += [
            {
                'sources': [
                    ...
                    {
                        'filepath': sp_ncso_complete_dst_path,
                        'bandnumber': 1,
                        'unpack_bits': False
                    },
                    {
                        'filepath': sp_nwso_complete_dst_path,
                        'bandnumber': 1,
                        'unpack_bits': False
                    }], \
    ...
    def remove_and_rename_final_layers(self):
        for dst_suffix_name in ['NOBS.tif']:
            file_path = os.path.join(
                self.workdir_path, self.ALGO_OUTPUT_FOLDER_NAME,
                self.product_folder_name_final,
                f'{self.product_folder_name_final}_{dst_suffix_name}'
            )
            if os.path.exists(file_path):
                print(f'remove_unused > {file_path}')
                os.remove(file_path)
        for dst_suffix_name in ['mean_QC.tif','median_QC.tif']:
            file_path = os.path.join(
                self.workdir_path, self.ALGO_OUTPUT_FOLDER_NAME,
                self.product_folder_name_final,
                f'{dst_suffix_name}'
    ...
    def sps2_product_main_layers_creation(self):
        ...
        rewrite_cog.rewrite_cog(
            ...
            os.path.join(
                self.workdir_path, self.ALGO_OUTPUT_FOLDER_NAME, self.product_folder_name_final
            )
        )
        add_quicklook.add_quicklook(
            os.path.join(self.workdir_path, self.ALGO_OUTPUT_FOLDER_NAME, self.product_folder_name_final),'_SCD.tif'
        )
        # self.make_metadata_and_xml_files()
        self.remove_and_rename_final_layers()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='This script is used to process SP S1S2 from a set of GFSCs on a tile on a hydrological year.'
    )
    parser.add_argument(
        "--workdir", type=str, required=True, help='work directory'
    )
    parser.add_argument(
        "--input-folder", type=str, required=True, help='input directory'
    )
    parser.add_argument(
        "--output-folder", type=str, required=True, help='output directory'
    )
    parser.add_argument(
        "--tile-id", type=str, default='', required=True, help='tile id'
    )
    parser.add_argument(
        "--start-date", dest="start_date", type=str, default='', required=True, help='start date'
    )
    parser.add_argument(
        "--end-date", type=str, default='', required=True, help='end date'
    )
    args = parser.parse_args()

    lis_fsc_pos_processing = LisSpS2PostProcessing(
        args.workdir, args.input_folder, args.output_folder, args.tile_id, args.start_date, args.end_date
    )
