import os, uuid, datetime, shutil, json, argparse, gfio, xmltodict, traceback
import numpy as np
...
def main():
    ...
    gfio.log("Temporal gap filling started",level="INFO")
    ...
    for productTitle in sysargv['gf1'] + sysargv['gf'] + sysargv['fsc']:
        try:
            ...
            if useMetadata or productType == 'GFSC1':
                productXml = xmltodict.parse(open(xmlFile,'r').read())
            else:
                # create fake metadata for FSC for rest of the steps
    ...
    if os.path.exists(productDir):
        gfio.log("Product already produced.",level="WARNING")
        return 0
    ...
    if productUniqueInputTitles != []:
        ...
        # Fill water pixels in GF and QC
        ...
        np.place(qc, (wl == PERMANENTWATER) + (wl == TEMPORARYWATER), WATER)
        ...
        try:
            ...
            gfio.writeRaster(os.path.join(productTmpDir,gfio.getFileName(productTitle,'QC.tif')),qc,geoTransform,projectionRef,colorMap=qcColorMap)
            ...
            if cogCheck == 1:
                ...
                return 61
            gfio.writeRaster(os.path.join(productTmpDir,gfio.getFileName(productTitle,'AT.tif')),ad,geoTransform,projectionRef,noData=0)
        ...
        gfio.log("No valid input data is found. Nothing to produce.",level="WARNING")
        gfio.log("Writing empty JSON Metadata file",level="WARNING")
        jsonFile = open(os.path.join(productTmpDir,'dias_catalog_submit.json'), 'w')
        jsonFile.close()
