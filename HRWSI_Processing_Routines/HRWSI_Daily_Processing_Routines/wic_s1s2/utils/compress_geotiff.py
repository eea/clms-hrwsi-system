...
def replicate_directory_with_compressed_tiffs(
        input_fol, output_fol=None, add_prefix=None, tiled=True, compress='deflate',
        zlevel=4, predictor=1, add_overviews=False, use_default_cosims_config=False,
        verbose=0
):
    '''
    TODO
...
def add_line_breaks(string_in, paragraph_max_len=50):
    ...
    if paragraph_max_len < 5:
        raise Exception('paragraph_max_len must be >= 5')
    if string_in.count('\n') > 0:
...
if __name__ == '__main__':
    ...
    replicate_directory_with_compressed_tiffs(args.input, output_fol=args.output, tiled=args.tiled, compress=args.compress, zlevel=args.zlevel, \
        predictor=args.predictor, add_overviews=args.add_overviews)
