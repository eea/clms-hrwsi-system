...
import os
import numpy as np
from common.basic_functions import logical_array_list_operation
from common.exitcodes import RuntimeInputFileError
from utils.compress_geotiff import compress_geotiff_file
try:
...
def bit_bandmath(
        output_file,
        raster_info_dict,
        source_list_per_band,
        no_data_values_per_band=None,
        compress=True,
        add_overviews=True,
    ...
    for band_no in range(nbands):
        ...
        for dico in source_list_per_band[band_no]:
            ...
            for i_src, src in enumerate(dico['sources']):
                local_name = f'A{i_src}'
                #get all input arrays
                assert os.path.exists(src['filepath']), f'file {src["filepath"]} missing'
                ds_loc = gdal.Open(src['filepath'])
                if ds_loc is None:
                    raise RuntimeInputFileError(f'gdal could not open file {src["filepath"]}')
...
def bit_bandmath_uint16(
    ...
    """Creates a copy of a monoband uint16 source model file and fills each bits independantly from different uint16 files.
    ...
    ds_out = gdal.GetDriverByName('GTiff').Create(output_file, raster_info_dict['size'][0], raster_info_dict['size'][1], nbands, gdal.GDT_UInt16)
    is_gcps = 'gcps' in raster_info_dict
    if 'gcps' in raster_info_dict:
        assert 'geoTransform' not in raster_info_dict
        ds_out.SetGCPs([gdal.GCP(el['x'], el['y'], el['z'], el['pixel'], el['line']) for el in raster_info_dict['gcps']['gcpList']], raster_info_dict['gcps']['coordinateSystem']['wkt'])
