...
def getMeteodataIndicesForTile(source_shape,source_transform,source_proj,target_shape,target_transform,target_proj):
    # Find indices to be used in the target raster, to get data from source raster
    # used in getting meteodata from meteoraster (targetted proj) in wgs into product raster (source proj) in UTM
    # our meteo data geotransform is for edges, thus use the offset of half a resolution
    # also, x,y and lat, lon are inverse
    # so do not use this method elsewhere

    sx = source_transform[0] + np.indices(source_shape,dtype=np.float32)[1]*source_transform[1]
    sy = source_transform[3] + np.indices(source_shape,dtype=np.float32)[0]*source_transform[5]

    transformer = Transformer.from_crs(source_proj, target_proj)
    ty, tx = transformer.transform(sx,sy)

    mc = np.round((tx - target_transform[0] - target_transform[1]/2.)/target_transform[1]).astype(np.int32)
    mr = np.round((ty - target_transform[3] - target_transform[5]/2.)/target_transform[5]).astype(np.int32)
...
def upscale(data,scale,noData=None,valueMin=None,valueMax=None, classes = []):
    ...
    if valueMin is None:
        valueMax is None
    log("Upscaling raster",level="DEBUG")
