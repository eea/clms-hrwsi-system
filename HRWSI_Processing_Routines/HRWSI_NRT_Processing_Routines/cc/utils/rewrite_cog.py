...
def rewrite_cog(src_path:str, dest_path=None, verbose=1) -> None:
    '''
    Make the product folder into a Cloud Optimized Geotiff (COG).

    :param src_path: the absolute path to the product folder.
    :param dest_path: the absolute path to the COG localisation. If not set the COG is made 
        next to the original product with the same name.
    :param verbose: if set to any value over 0 print logs.
    ...
    if os.path.isdir(src_path):
        for root_src, dirs, files in os.walk(src_path, topdown=True):
            if dest_path is not None:
                root_target = os.path.abspath(root_src).replace(
                    os.path.abspath(src_path),
                    os.path.abspath(dest_path)
                )
            else:
                root_target = os.path.abspath(root_src)
                ...
                if src_file.lower().split('.')[-1] in ['tiff', 'tif']:
                    if int(verbose) > 0:
                        if dest_path is None:
                            print(f"rewrite_cog: {src_file} (inplace)")
                        else:
                            print(f"rewrite_cog: {src_file} -> {target_file}")
                    rewrite_cog(src_file, dest_path=target_file, verbose=0)
                elif dest_path is not None:
                    if int(verbose) > 0:
                        print("copy: {src_file} -> {target_file}")
    ...
    raster_data = band_src.ReadAsArray()
    ...
    try:
        ...
        if dest_path is not None:
            if verbose > 0:
                print(f'rewrite_cog: {src_path} -> {dest_path}')
            shutil.move(temp_file, dest_path)
        else:
            if verbose > 0:
                print(f'rewrite_cog: {src_path} (inplace)')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Compress all TIFF files within a folder",
    ...
    group.add_argument(
        ...
        action='store_true', 
        default=False, 
        help='In place compression in input_folder. This is not recommended since it does ' + \
        'not provide a means to save disk space during processing : it creates all new ' + \
        'data in a temporary output_folder and then deletes the input_folder and moves ' + \
        'the temporary output_folder in place of the input_folder. It is therefore only ' + \
        'a convenience option. If this option is used and output_folder specified, an error ' + \
        'is raised.'
    )
