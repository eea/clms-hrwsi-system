...
class FileUtil(object):
    ...
    def create_dir(path:str, overwrite:bool=False, error_on_already_existing:bool=False)->None:
        ...
        if overwrite and os.path.exists(path):
            logging.warning('Removing already existing directory %s, its subfolders and files.', path)
            shutil.rmtree(path)

        try:
            os.makedirs(path, exist_ok=True)
        except PermissionError as error:
            logging.critical('Directory %s cannot be created as due to permission issues.', path)
            logging.critical('%s', error)
            raise error
    ...
    def create_file_path(path:str, overwrite:bool=False, error_on_already_existing:bool=False)->None:
        ...
        if not dir_path:
            logging.error('No directory path can be extracted from %s, aborting directory creation', path)
            return
        ...
        if overwrite and os.path.exists(dir_path):
            logging.warning('Removing already existing folder %s, its subfolders and files.', dir_path)
    ...
    def create_file(path, overwrite:bool=False, error_on_already_existing:bool=False)->None:
        ...
        try:
            logging.info('Creating file at location %s', path)
            ...
            logging.critical('File %s cannot be created as due to permission issues.', path)
            logging.critical("%s", error)
            raise error
        except Exception as error:
            logging.critical('File %s cannot be created as due to unexpected error.', path)
            logging.critical("%s", error)
            raise error
